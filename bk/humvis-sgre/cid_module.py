...
class GFD(nn.Module):
    def __init__(self, cfg):
        ...
        self.deckpt = nn.Linear(self.in_channels, self.num_keypoints*2)
        self.decpose = nn.Linear(self.in_channels, 24*6)
        self.decshape = nn.Linear(self.in_channels, 10)
        self.deccam = nn.Linear(self.in_channels, 3)
        ...
        self.parents = self.smpl.parents.tolist()[:24]
    ...
    def forward(self, features, instances, input_scale=1.0):
        ...
        bs = pred_instance_heatmaps.shape[0]
        # smpl mesh
        gt_location = torch.flip(instances['instance_coord'], [1])
        pred_kpts = self.deckpt(instance_params).reshape(bs, self.num_keypoints, 2) + gt_location[:, None, :]
        pred_kpts = pred_kpts.reshape(bs, self.num_keypoints*2)

        pred_pose_6d = self.decpose(instance_params)
        pred_shape = self.decshape(instance_params)
        pred_cam = self.deccam(instance_params)
        cam_trans = self.get_camera_trans(pred_cam, 512*input_scale)
        global_rotmats = rot6d_to_rotmat(pred_pose_6d.reshape(-1, 6)).reshape(-1, 24, 3, 3)
        assert global_rotmats.shape[0] == bs
        # convert global rotmat into rel rotmat
        root_rotmat = global_rotmats[:, 0, :, :]
        rel_rotmats = [root_rotmat]
        for i in range(23):
            i_val = int(i + 1)
            joint_rotmat = global_rotmats[:, i_val, :, :]
            parent = self.parents[i_val]
        ...
        pred_pose_axis = rotmat_to_axis_angle(rel_rotmats).reshape(bs, 24*3)
        pred_outputs = self.smpl(pred_pose_axis.contiguous(), pred_shape)
        smpl_mesh_cam = pred_outputs.vertices + cam_trans[:, None, :]

        return pred_instance_heatmaps, pred_instance_masks, F.softmax(pred_instance_parts, dim=1), smpl_mesh_cam
